...
export default function ApprovalsPage() {
  ...
  const [approveAccount, setApproveAccount] = useState("public")
  ...
  const handleApprove = async (id: string) => {
    try {
      const res = await fetch(`/api/expenses/${id}/approve`, {
        ...
        body: JSON.stringify({ account: approveAccount }),
      })
  ...
  return (
    <div className="min-h-screen bg-gray-50">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between h-16">
            <div className="flex items-center">
              ...
              <h1 className="text-xl font-bold text-gray-900">报销审批</h1>
            </div>
          </div>
        </div>
      </nav>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {message && (
          <div className={`p-3 rounded mb-4 ${message.includes("成功") || message.includes("通过") || message.includes("拒绝") ? "bg-green-50 text-green-700" : "bg-red-50 text-red-700"}`}>
            {message}
          </div>
        )}
        ...
        <div className="bg-white rounded-lg shadow p-4 mb-6">
          ...
          <div className="mt-3 flex justify-between items-center">
            <button
              onClick={() => {
                setFilters({ projectId: "", submitterId: "", search: "", dateFrom: "", dateTo: "" })
                setPagination(prev => ({ ...prev, page: 1 }))
              }}
        ...
        <div className="bg-white rounded-lg shadow">
          <div className="px-6 py-4 border-b border-gray-200">
            <h2 className="text-lg font-semibold">待审批报销</h2>
          </div>
          <div className="divide-y divide-gray-200">
            {loading ? (
              <div className="px-6 py-8 text-center text-gray-500">加载中...</div>
            ) : expenses.length === 0 ? (
              <div className="px-6 py-8 text-center text-gray-500">暂无待审批报销</div>
            ) : (
                ...
                <div key={expense.id} className="px-6 py-4">
                  <div className="flex justify-between items-start mb-4">
                    <div>
                      ...
                      <div className="text-sm text-gray-500">
                        ...
                        时间：{new Date(expense.createdAt).toLocaleDateString("zh-CN")}
                      </div>
                  ...
                  <div className="flex items-center space-x-2">
                    {approvingId === expense.id ? (
                      ...
                      <button
                        onClick={() => setApprovingId(expense.id)}
                        className="bg-green-600 text-white px-4 py-2 rounded-md hover:bg-green-700 text-sm"
                      >
                        通过
                      </button>
